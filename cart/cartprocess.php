...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $size = isset($_POST['size']) ? $_POST['size'] : 'Standard'; // Default if not set
    $status = 'In Cart'; // Default status when adding to cart
    ...
    $check_sql = "SELECT cart_id, quantity FROM cart 
                 WHERE user_id = ? AND product_id = ? AND color = ? AND size = ? AND status = 'pending'";
